...
class protocol_counters(object):
    ...
    def add(self,protocol, portnumber='*'):
        if protocol in self.protocol_array:
            self.protocol_array[protocol].update([portnumber])
        else:
            self.protocol_array['Unknown'].update(['*'])
...
filename=sys.argv[1] if len(sys.argv)>1 else "kern.log"
